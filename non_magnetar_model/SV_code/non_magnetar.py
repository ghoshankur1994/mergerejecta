...
import non_magnetar_beta1 as NM1
import non_magnetar_beta2 as NM2
...
def nuc(T):
 ...
 return np.log10(vc)
'''
plt.loglog(beta_fun(t),1.+gamma_m(t))
plt.loglog(beta_fun(t),gamma_m(t))
...
tm=optimize.brentq(fun_root,1e-20,500000.)    ##### time where nua = num for nua>num case
print (tm)
...
teq=optimize.brentq(fun_root2,1e-15,500000.) 
...
for i in range(np.size(t)):
 ...
 f3[i]=num(t[i])
 f4[i]=nuc(t[i])
 f5[i]=10**NM1.flux(np.log10(1.4e9),t[i])
 f6[i]=10**NM2.flux(np.log10(1.4e9),t[i])
 #f7[i]=10**Ism.flux(np.log10(0.6e9),t[i])



plt.loglog(t,f5,color='#E90BF3',linewidth=3.0,label=r'$\beta_o=0.3$')
plt.loglog(t,f6,color='#9FF425',linewidth=3.0,label=r'$\beta_o=0.5$')
